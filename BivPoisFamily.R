...
PoissonMVlogLambda1 <- function(lambda1 = NULL, lambda2 = NULL, lambda3 = NULL, y2 = y2, scale.grad = TRUE) {
  ...
  loss <- function(lambda2 = lambda2, lambda3, y, y2, f = f) {
    ...
    for (i in 1:length(y2)) {
      ...
      intm.res <- -((-exp(f[i]) - lambda2[i] - lambda3[i]) + (y[i] * f[i] - log(factorial(y[i]))) + 
                      (y2[i] * log(lambda2[i]) - log(factorial(y2[i]))) + log(sum((choose(y[i], kindex) * choose(y2[i], 
                                                                                                                 kindex) * factorial(kindex)) * (lambda3[i]/(exp(f[i]) * lambda2[i]))^kindex)))
      res <- c(res, intm.res)
    }
    res
  }
  ...
  ngradient <- function(y, f, w = 1) {
    ...
    for (i in 1:length(y2)) {
      kindex <- 0:min(y[i], y2[i])
      intm.res <- y[i] - exp(f[i]) - (sum(kindex * factorial(kindex) * exp(-f[i] * kindex) * choose(y[i], 
                                                                                                    kindex) * choose(y2[i], kindex) * lambda3[i]^kindex/(lambda2[i]^kindex))/sum(exp(-f[i] * 
...
PoissonMVlogLambda2 <- function(lambda1 = NULL, lambda2 = NULL, lambda3 = NULL, y1 = y1, scale.grad = TRUE) {
  ...
  loss <- function(lambda1 = lambda1, lambda3, y, y1, f = f) {
    ...
    for (i in 1:length(y)) {
      ...
      intm.res <- -((-lambda1[i] - exp(f[i]) - lambda3[i]) + (y1[i] * log(lambda1[i]) - log(factorial(y1[i]))) + 
                      (y[i] * f[i] - log(factorial(y[i]))) + log(sum((choose(y1[i], kindex) * choose(y[i], 
                                                                                                     kindex) * factorial(kindex)) * (lambda3[i]/(lambda1[i] * exp(f[i])))^kindex)))
      res <- c(res, intm.res)
    }
    res
  }
  ...
  ngradient <- function(y, f, w = 1) {
    ...
    for (i in 1:length(y)) {
      mink <- min(y[i], y1[i])
      kindex <- 0:mink
      intm.res <- y[i] - exp(f[i]) - (sum(kindex * exp(-f[i] * kindex) * factorial(kindex) * choose(y[i], 
                                                                                                    kindex) * choose(y1[i], kindex) * lambda3[i]^kindex/(lambda1[i]^kindex))/sum(exp(-f[i] * 
...
PoissonMVlogLambda3 <- function(lambda1 = NULL, lambda2 = NULL, lambda3 = NULL, y2 = y2, scale.grad = TRUE) {
  ...
  loss <- function(lambda1, lambda2, y, y2, f = f) {
    ...
    for (i in 1:length(y)) {
      ...
      intm.res <- -((-lambda1[i] - lambda2[i] - exp(f[i])) + (y[i] * log(lambda1[i]) - log(factorial(y[i]))) + 
                      (y2[i] * log(lambda2[i]) - log(factorial(y2[i]))) + log(sum((choose(y[i], kindex) * choose(y2[i], 
                                                                                                                 kindex) * factorial(kindex)) * (exp(f[i])/(lambda1[i] * lambda2[i]))^kindex)))
      res <- c(res, intm.res)
    }
    res
  }
  ...
  ngradient <- function(y, f, w = 1) {
    ...
    for (i in 1:length(y)) {
      mink <- min(y[i], y2[i])
      kindex <- 0:mink
      intm.res <- -exp(f[i]) + (sum(kindex * exp(f[i] * kindex) * factorial(kindex) * choose(y[i], 
                                                                                             kindex) * choose(y2[i], kindex)/(lambda1[i]^kindex * lambda2[i]^kindex))/sum(exp(f[i] * 
